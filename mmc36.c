...
int main(int ile, char *tab[])
{
 ...
 //  FILE *model_j;
   ...
   double chikw,Granice,obs_jd; //,czas;
int jaki_plik=0;
int co_ile=1;
double dop_g[9];
...
rewind(dane); 
 ...
 sscanf(tab[4],"%d",&jaki_plik);
     for(e=0;e<d;e++)   //wczytuje plik z danymi do tablicy dane_t
     {
      if(jaki_plik==1)// dodano 2012-12-10 15:20:28 wybor w 4 prametrze jaki rodzaj pliku podano  zm 26.04.2013 13:10:17 
      {
      ...
      {
      fscanf(dane,"%lf%lf%3s\n",&obs_jd,&dane_t[e].w,pri); //plik jak w symulacjach w matlabie z dodanymi pri albo sec w ostniej kolumnie 15.11.2012 11:33:15 
      dane_t[e].jd=obs_jd+2400000.0; //to poniewarz w pliku z up jest 7 cyfr jd a w matlabie 5
      }
