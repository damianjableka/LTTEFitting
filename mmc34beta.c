...
if(ile>=6) //jesli podamy kolejny argument to funkcja wczyta z pliku o takije nazwie rozklad poczatkowy
 {
  los=fopen(tab[5],"r");
    for(d=0;d<Ile_los;d++)   //d numeruje losowania
     {
    fscanf(los,"%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",&dop_t[d][1],&dop_t[d][2],&dop_t[d][3],&dop_t[d][4],&dop_t[d][5],&dop_t[d][6],&dop_t[d][7],&dop_t[d][8],&dop_t[d][9],&dop_t[d][10],&dop_t[d][0]);
     }
    fclose(los);
  }    
 else
 { //losowanie rozkladu poczatkowego jesli nie jest podany 5 argument
	...
	ep=epoka(dane_t[c].jd,dop_t[d][1],dop_t[d][2],dane_t[c].ps);
        chikw+=pow((Y(dane_t[c].jd,dop_t[d][1],dop_t[d][2],ep)-qLITE(dane_t[c].jd,dop_t[d][1],dop_t[d][2],dop_t[d][3],dop_t[d][4],dop_t[d][5],dop_t[d][6],dop_t[d][7],dop_t[d][8],dop_t[d][9],dop_t[d][10]))*dane_t[c].w,2);
...
//U fprintf(rand_seed,"%d\t%d\n",srand1,srand2);  //U wypisuje rozklad ziarna srand zwalnia proces do usuniecia
	...
	ep=epoka(dane_t[c].jd,sec_t[1],sec_t[2],dane_t[c].ps);
  	...
  	chikw+=pow((Y(dane_t[c].jd,sec_t[1],sec_t[2],ep)-qLITE(dane_t[c].jd,sec_t[1],sec_t[2],sec_t[3],sec_t[4],sec_t[5],sec_t[6],sec_t[7],sec_t[8],sec_t[9],sec_t[10]))*dane_t[c].w,2);
...
if(f%Coile==0) //co coile zawerzen granic
{
printf("OUT\n jack %d z %d danych\n",jack,ilosc_danych);
...
rozklad=fopen("rozklad","w");
   for(d=0;d<Ile_los;d++) 
    {//double qLITE(double JD,double JD0,double P,double q,double p3,double A,double ecc,double T0,double w,double p,double r) //fcja opisujaca LITE+qE^2
     fprintf(rozklad,"%lf\t%lf\t%e\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",dop_t[d][1],dop_t[d][2],dop_t[d][3],dop_t[d][4],dop_t[d][5],dop_t[d][6],dop_t[d][7],dop_t[d][8],dop_t[d][9],dop_t[d][10],dop_t[d][0]);
    }  
...
wykres=fopen("wykres","w");
	...
	for(e=0;e<ile_danych;e++)
	 {
	  ql=qLITE(dane_t[e].jd,dop_t[0][1],dop_t[0][2],dop_t[0][3],dop_t[0][4],dop_t[0][5],dop_t[0][6],dop_t[0][7],dop_t[0][8],dop_t[0][9],dop_t[0][10]);
...
jack_f=fopen("jack","a");
 fprintf(jack_f,"%lf\t%lf\t%e\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",dop_t[0][1],dop_t[0][2],dop_t[0][3],dop_t[0][4],dop_t[0][5],dop_t[0][6],dop_t[0][7],dop_t[0][8],dop_t[0][9],dop_t[0][10],dop_t[0][0]);

int uu;
for(uu=0;uu<11;uu++) 
  jack_a[jack][uu]=dop_t[0][uu];

 for(dpt=0;dpt<11;dpt++) //srednia
...
// wychodzi na to ze nie ma sensu
